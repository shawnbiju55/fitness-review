...
- Always pipe output-heavy commands through `| Out-String` to prevent PowerShell pager blocking.
- Always use `git --no-pager` prefix for all git diff/log/show commands.
...
- Always use `git --no-pager diff` — never `git diff` — to prevent pager blocking in PowerShell.
